...
/** Countdown tick — sharp ascending beep */
export function generateCountdownTick(ctx) {
  return renderBuffer(ctx, 0.15, (data, sr) => {
    for (let i = 0; i < data.length; i++) {
      const t = i / sr;
      const env = Math.exp(-t * 20) * Math.min(t * 100, 1);
      data[i] =
...
/** Countdown GO! — bright major chord burst */
export function generateCountdownGo(ctx) {
  return renderBuffer(ctx, 0.4, (data, sr) => {
    for (let i = 0; i < data.length; i++) {
      const t = i / sr;
      const env = Math.min(t * 50, 1) * Math.exp(-t * 4);
      data[i] =
...
/** Combo milestone — rising sparkle chime */
export function generateComboMilestone(ctx) {
  return renderBuffer(ctx, 0.25, (data, sr) => {
    for (let i = 0; i < data.length; i++) {
      const t = i / sr;
      const env = Math.min(t * 80, 1) * Math.exp(-t * 12);
      const sweep = 800 + t * 2000;
      data[i] =
        (Math.sin(2 * Math.PI * sweep * t) * 0.2 +
          Math.sin(2 * Math.PI * sweep * 1.5 * t) * 0.1) *
        env;
    }
  });
}

/** Victory fanfare — triumphant ascending arpeggio + sustained chord */
export function playVictoryFanfare(ctx) {
  if (!ctx) return;
...
/** Defeat dirge — descending minor passage */
export function playDefeatDirge(ctx) {
  if (!ctx) return;
  const master = ctx.createGain();
  master.gain.value = 0.25;
  master.connect(ctx.destination);

  const notes = [587.33, 523.25, 466.16, 440.0];
  const now = ctx.currentTime;
  ...
  const chordFreqs = [146.83, 174.61, 220.0];
